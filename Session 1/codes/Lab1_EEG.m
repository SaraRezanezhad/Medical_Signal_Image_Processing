%% Load dataset
% Mohamad Hosein Faramarzi - Ali Khosravipour - Sara Rezanezhad 
...
for i = 1:size(ranges, 1)
    ...
    subplot(4, 1, i);
    plot(rangeTime, rangeData);
    grid on;
    xlabel('Time (s)');
    ylabel('Amplitude (\muV)');
    title(['EEG Channel5-Time ' num2str(ranges(i,1)) '-' num2str(ranges(i,2)) 's'], 'FontSize',8);
    grid on;  
...
for i = 1:size(ranges, 1)
    ...
    xlabel('Time (s)', 'FontSize', 10);
    ylabel('Amplitude (\muV)', 'FontSize', 10);
    title(['EEG ChannelC3-Time ' num2str(ranges(i,1)) '-' num2str(ranges(i,2)) 's'], 'FontSize', 12);
...
for i = 1:size(ranges, 1)
    ...
    title(['Frequency Spectrum for EEG Channel:C3- Time ' num2str(ranges(i,1)) '-' num2str(ranges(i,2)) 's'], 'FontSize', 12);
...
for i = 1:size(ranges, 1)
    ...
    title(['Welch PSD for EEG Channel:C3- Time ' num2str(ranges(i,1)) '-' num2str(ranges(i,2)) 's'], 'FontSize', 12);
...
for i = 1:size(ranges, 1)
    ...
    [~, F, T, P] = spectrogram(rangeData, window, Noverlap, nfft, samplingRate);
    ...
    title(['Spectrogram for EEG Channel:C3- Time ' num2str(ranges(i,1)) '-' num2str(ranges(i,2)) 's'], 'FontSize', 12);
