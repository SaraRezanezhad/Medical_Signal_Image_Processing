...
%load("fecg1.dat");
load("D:\term9\SignalLab\New folder\Lab3Codes_99101502_99104095_99101643\fecg2.dat");

%load("mecg1.dat");
load("D:\term9\SignalLab\New folder\Lab3Codes_99101502_99104095_99101643\mecg2.dat");
%load("noise1.dat");
load("D:\term9\SignalLab\New folder\Lab3Codes_99101502_99104095_99101643\noise2.dat");
load("D:\term9\SignalLab\New folder\Lab3Codes_99101502_99104095_99101643\observation.dat");
load("D:\term9\SignalLab\New folder\Lab3Codes_99101502_99104095_99101643\X.dat");
%% ICA implementation on observation X
 XT=X.';
[W,Z_hat]=ica(XT);
A_apprx=inv(W);
save("W_P3","W");
save("Z_hat_P3","Z_hat");
save("A_apprx_P3","A_apprx");
plot3ch(X,256,"Scatter plot");
plot3dv(A_apprx(:,1));
plot3dv(A_apprx(:,2));
plot3dv(A_apprx(:,3));
%% Plot Z_hat
...
figure; 
hold on; 
plot(t, Z_hat(1, :), 'Color', [0 0.4470 0.7410], 'LineWidth', 2); 
plot(t, Z_hat(2, :), 'Color', [0.9290 0.6940 0.1250]		, 'LineWidth', 2); 
plot(t, Z_hat(3, :), 'Color', [0.6350 0.0780 0.1840], 'LineWidth', 2); 
xlabel('Time or Sample Points'); 
ylabel('Amplitude'); 
title('Plot of Z\_hat Components from ICA');
...
N=size(A_apprx,2);
W_Fetus(:,1:N-1)=0;
X_Fetus=W_Fetus*Z_hat;
Xdenoised =A_apprx(:,3) * Z_hat(3,:);

figure; 
hold on; 
plot(t, X_Fetus(3,:), 'Color', [0.6350 0.0780 0.1840], 'LineWidth', 2); % Row 3: cyan
xlabel('Time or Sample Points'); 
ylabel('Amplitude'); 
title('Plot of Fetus ECG Time Series');
grid on;
xlim([1 2560]);
hold off; 
%% 
